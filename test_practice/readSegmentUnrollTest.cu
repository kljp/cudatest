#include "../../prof_cuda_code_sample/common/common.h"
...
__global__ void readOffsetUnroll4inTextbook(float *A, float *B, float *C, const int n, int offset){
...
int main(int argc, char **argv)
{
    ...
    CHECK(cudaGetLastError());
	 ...
	 // kernel 4
	 iStart = seconds();
	 readOffsetUnroll4inTextbook<<<grid.x / 4, block>>>(d_A, d_B, d_C, nElem, offset);
	 CHECK(cudaDeviceSynchronize());
	 iElaps = seconds() - iStart;
	 printf("unroll4real <<< %4d, %4d >>> offset %4d elapsed %f sec\n",
			 grid.x / 4, block.x, offset, iElaps);
	 CHECK(cudaGetLastError());
